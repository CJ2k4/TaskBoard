...
**M0, M1, M2 done front to back; M3 backend done; M3 frontend Pass A (card drag) done.** Next up is **M3 Pass B** — dragging *columns* to reorder (`PATCH /api/columns/{id}/move`, `@dnd-kit` horizontal sortable); then M4 (roles) before M5 (real-time).
