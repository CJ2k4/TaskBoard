...
**M0, M1, and M2 are done, front to back.** Next up is **M3** — drag-and-drop *moves* (the `move` endpoints + `@dnd-kit`) on top of the ranks already stored.

- `server/` — Spring Boot backend. Postgres wired via `docker-compose.yml`, Flyway migrations `V1__baseline` (empty) + `V2__app_user` + `V3__board_graph` (board, board_membership, board_column, card). Auth is complete (register/login/refresh JWTs, JWT filter, `GET /api/me`). **M2 board CRUD is complete**: `/api/boards` (POST/GET/GET{id}/PATCH/DELETE, where `GET /{id}` returns the whole board nested), `/api/boards/{id}/columns` + `/api/columns/{id}` (PATCH/DELETE, delete blocked if non-empty), `/api/columns/{id}/cards` + `/api/cards/{id}`. Ordering uses `common/ranking/LexoRank` (server-authoritative; new items appended). Access is **owner-scoped** via `BoardService.requireOwnedBoard` (non-owner → 404 to hide existence); the owner's `OWNER`/`ACTIVE` `board_membership` row is written on board creation so M4 can switch to role checks with no backfill. Also `GET /api/health` (M0), a global exception handler (404/403/409/401/400/500 → `ApiError`), and CORS for the Next.js origin. **58 tests**; `board/` mirrors the `auth/` feature-first/layered layout.
