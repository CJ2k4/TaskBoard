...
**M0, M1, M2 are done front to back; the M3 backend (move endpoints) is done.** Next up is the **M3 frontend** — `@dnd-kit` drag UI with optimistic moves reconciling to the server's returned rank (honor `frontend/AGENTS.md`: read the bundled Next.js 16 docs first).
