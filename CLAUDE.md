...
**M0, M1, and the M2 backend are done.** Next up is the **M2 frontend** (board list, board detail rendering columns/cards, create/rename/delete UI); then M3 adds drag-and-drop *moves* on top of the ranks already stored.
...
- `frontend/` — Next.js 16 App Router app (`src/app`, `src/lib`, `src/components`). Auth UI is complete: register/login pages, an `AuthProvider` context, a `<Protected>` route guard, and a dashboard. Landing page (`/`) still shows the backend health status. **No board UI yet** — that's the next phase.
